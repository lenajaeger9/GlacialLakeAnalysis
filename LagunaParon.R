# ------------------------------------------------------------------------------------#
## ---------------------Laguna Paron Area Change Analysis----------------------------##
# ------------------------------------------------------------------------------------#
...
lsat_path <- "path/to/folder"
aoi_extent <- st_read("path/to/file/laguna_paron.gpkg")

# Loop to import Landsat scenes, rename bands, crop and stack them and save them in seperate folder

# for loop for Landsat 8
...
# Overview over the Lsat data
lsat_files <- list.files(lsat_path, full.names = T)


# load individual preprocessed files
lsat_1989 <- rast("path/to/file/data/FinalData/laguna_paron/19890824.tif")
lsat_1994 <- rast("path/to/file/data/FinalData/laguna_paron/19940721.tif")
lsat_1999 <- rast("path/to/file/data/FinalData/laguna_paron/19990804.tif")
lsat_2010 <- rast("path/to/file/data/FinalData/laguna_paron/20100818.tif")
lsat_2014 <- rast("path/to/file/data/FinalData/laguna_paron/20140712.tif")
lsat_2019 <- rast("path/to/file/data/FinalData/laguna_paron/20190710.tif")
lsat_2022 <- rast("path/to/file/data/FinalData/laguna_paron/20220702.tif")
...
polygon_2010 <- as.polygons(water_mask_ndgi$w_1989)
plot(polygon_2010)

lake_outlines <- list()
lake_names <- c("l_1989", "l_1994", "l_1999", "l_2010", "l_2014", "l_2019", "l_2022")

for (i in 1:length(water_mask_ndgi)) {
  indi_lake <- as.polygons(water_mask_ndgi[[i]])
...
ggplot(pixels_df, aes(x= year, y= area_sqkm)) +
  geom_point() +
  labs(title = "Area of Laguna Paron Over Time",
       x = "Year",
       y = "Area [km²]") +
